...
export function renderInit(results: readonly InitResult[]): string {
  ...
  lines.push(
    ...
    '  1. issueforge doctor',
    '',
    '  Try one run before automating anything — this needs no runner and no',
    '  workflow, and is the fastest way to see what a report looks like:',
    '',
    '       issueforge run reproduce --repo <owner>/<repo> --issue <n>',
    '',
    '  To have a label start runs on its own:',
